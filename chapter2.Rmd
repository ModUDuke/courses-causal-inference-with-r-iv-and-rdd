...
`@feedback`
- This is perfectly fine with IV analysis. If the measurement error is not affecting the measurement of the treatment and outcome variables, then we're safe. We are looking for the option that would violate the IV assumptions, so try again.
- Correct! This would break our IV analysis, because it would mean that the instrument has some direct effect on the values of the outcome variable by itself, but to use IV correctly, we need the instrument to only indirectly act on the outcome via the treatment variable.
- This is perfectly fine with IV analysis. We expect the instrument to have a causal effect on the treatment variable, so it would make sense that the measurement error would show up in the value of our treatment variable. But we are looking for the option that would violate the IV assumptions, so try again.
...
- Some kids are happier to ride in bike lanes and happier to bike off-road, but those preferences cancel each other out

`@feedback`
- Correct! This is the best argument because, if kids only bike when there are bike lanes, then this argument would suddenly become a very weak one, because simply painting new lines on a road would make kids very happy, independently of whether they actually ride a bike or not.
- This might be true, but that is directed more at whether the treatment (riding with a family) affects the outcome (child happiness), not the instrument on the outcome like the question asks. Try again.
- Kids may need to feel safe to be happy, but we aren't trying to use a 'feeling of safety' as an instrument. Try again.
- If this were true, it sounds like the instrument would be uncorrelated with our outcome, which would make our IV analysis invalid, because we need the instrument to be correlated to the outcome exclusively by the causal effect of the treatment. So try again.
...
# Simulate opt-in behavior based on rainy weather
  draw <- runif(n)
  draw2 <- runif(n)
  ...
  # Simulate opt-in behavior based on windy weather
  draw <- runif(n)
  draw2 <- runif(n)
  CreditCo$windy <- draw>0.5
  xb   <- -.001*CreditCo$windy
  p    <- exp(xb)/(1+exp(xb))
  CreditCo$opt_in[CreditCo$offered==1] <- as.integer(draw2[CreditCo$offered==1]<p[CreditCo$offered==1])
  CreditCo$optin_p <- p
...
# Remove elements and variables from environment
  ...
  CreditCo <- CreditCo[CreditCo$offered==1,c("id","opt_in","FICO","female","race_white","default_pre","default_post","balance_pre","balance_post","windy","rainy")]
```
...
#  `windy` - TRUE if there were high wind speeds at customer's house
...
#2) First we need to determine whether our instrument satisfies the Relevance Assumption, so let's start with the variable `windy`, which tracks whether it was unusually windy at the respondents house. We are looking for a regression coefficient that is not zero, and that is also statistically significant. First let's run a simple regression of the variable `windy` on the variable `opt_in` in the CreditCo dataframe:

    Solution2 <- lm( ~ ,data=CreditCo)
    summary()

# 3) If that regression does show a correlation between opt-in status and windy weather, we need to make sure it's a statistically significant one. We will use the typical standard and hope that the p-value is under 0.05. What is the p-value associated with the t-test on the coefficient of windy? Enter in the p-value for this t-test as Solution3:

    Solution3 <-

# That p-value is far above our cutoff of .05, and it shows that the variable `windy` is not statistically correlated with the respondent's opt-in behavior. Therefore we must say that it fails the Relevance Assumption, and we can't use it as an instrument. But that still leaves us with the rainy part of this stormy weather as a potential instrument, so let's try that next.

# 4) Now let's find out if the variable `rainy` is a better candidate to be an instrumental variable. Using a linear regression, compute the correlation coefficient between rain and opt-in status, and then run the summary() command to see if there is a correlation between the instrument and our treatment.

    Solution4 <- lm( ~ ,data=CreditCo)
    summary()

# 5) If that regression does show a correlation between opt-in status and rainy weather, we need to make sure it's a statistically significant one. What is the p-value associated with the t-test on the coefficient of rain? Enter in the p-value for this t-test as Solution3:

    Solution5 <-

# 6) What is the sign of the correlation between rain and opt-in? You can write "positive", "negative", or "no correlation" into Solution4 as the answer:
...
`@solution`
```{r}
Solution2 <- lm(opt_in~windy,data=CreditCo)
Solution3 <- 0.67
Solution4 <- lm(opt_in~rainy,data=CreditCo)
Solution5 <- 0.0449
Solution6 <- "negative"
```
...
success_msg("Good work! The variable `rainy` does seem to have a statistically significant negative correlation with our outcome variable. Because we can say that it satisfies the Relevance Assumption, this is one important pillar that supports our argument that we can use the rainy weather as an instrument in this analysis.")
```
...
`@feedback`
- Not quite. Remember that relevance is refutable, so people don't usually complain about this because it can be checked in the data. Try again.
- This is probably not the most common complaint, because it would take a long and complex argument to justify this as the single main reason why the IV analysis is invalid. Try again.
- Correct! This would be the most common complaint, and it's a good one. Luckily, the example was theoretical!
...
`@sample_code`
```{r}
# The dataset `CreditCo` is available in your workspace. Once again, here is a data dictionary to help you make sense of the variable names and what they are representing:
...
# 4) Based on our use of the instrumental variable of `rainy`, did opting in to the credit offer increase or decrease credit balances of CreditCo's customers? (to Solution 4, type "increase" or "decrease")
...
`@feedback`
- Try again
- Correct! The previous coding examples happened to yield identical results because of the specific situation. In general, the results should yield numbers that are "statistically similar," that is, the confidence interval of one should contain the other.
