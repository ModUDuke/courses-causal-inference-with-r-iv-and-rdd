...
#2) First we need to determine whether our instrument satisfies the Relevance Assumption, so let's start with the variable `windy`, which tracks whether it was unusually windy at the respondents house, which would 
...
# That p-value shows that the variable `windy` is not statistically correlated with the respondent's opt-in behavior, so we must say that it fails the Relevance Assumption and we can't use it as an instrument. But that still leaves us with the rainy part of this stormy weather, so let's try that next.
...
# Remove elements and variables from environment
  rm(draw,frac_female,frac_treated,frac_white,n,p,xb)
  CreditCo <- CreditCo[CreditCo$offered==1,c("id","opt_in","FICO","female","race_white","default_pre","default_post","balance_pre","balance_post","rainy","cloudy")]
```
