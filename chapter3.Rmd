...
![](https://assets.datacamp.com/production/repositories/4496/datasets/e78d0e4d07a638e90e72cabf18dac6b2ff769d47/Rplot05.png)
...
`@feedback`
- We have a pretty healthy sample of NBA games, all teams are included both at home and away, and we are looking at games that are within 1 or 2 points at halftime, so in theory the teams are going to be of roughly equal quality on that day. Try again.
- Good job! This is an example of why conclusions from RDD analyses are only valid at the boundary of the discontinuity! If we get too far away, we will be picking up effects that will confound our causal inference.
- This actually has a basis in fact, but we are avoiding the home/road differential in team performance by looking at just the results of the home games for a team, not of both home and road games. Try again.
...
`@feedback`
- At least one of them is a sharp regression discontinuity design. Try again.
- This is not entirely correct: for example, in (3) the treatment boundary varies a bit across units, so it is fuzzy. Try again.
- Correct! Often the distinction between sharp and fuzzy hinges on the definition of treatment. For (1) and (2), treatment is defined as **eligibility** rather than takeup. In contrast (3) is fuzzy because the treatment boundary varies a bit across units. Rural districts may get 1 representative for every 200,000 people, but in urban districts, 1 politician may represent 600,000 people. In a way, the way that governments create districts often means they are not reacting proportionally to the changes in the population in particular regions, and it's often helpful to think of fuzzy RDDs as having a non-compliance problem.
- This is not entirely correct: for example, in (1) the treatment is defined as **eligibility** rather than takeup, so it is a sharp regression discontinuity design. Try again.
- This is not entirely correct: for example, in (2) the treatment is defined as **eligibility** rather than takeup, so it is a sharp regression discontinuity design. Try again.
- This is not entirely correct: for example, in (2) the treatment is defined as **eligibility** rather than takeup, so it is a sharp regression discontinuity design. Try again.
- This is not entirely correct: for example, in (3) the treatment boundary varies a bit across units, so it is fuzzy. Try again"
- At least one of them is a fuzzy regression discontinuity design. Try again.
